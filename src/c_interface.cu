...
#define DEFINE_VEC_INTERFACE(SUFFIX, TYPE)                                    \
    ...
    void vec_gather_##SUFFIX(void* h_src, void* h_map, void* h_dst) {         \
        ...
        if (src->device_ptr() == dst->device_ptr()) {                         \
            fprintf(stderr, "[C++ Error] vec_gather: src and dst must differ!\n"); \
            exit(1);                                                          \
        }                                                                     \
...
extern "C" {
    ...
    void device_synchronize() {
        CUDA_CHECK(cudaStreamSynchronize(GPU::DeviceEnv::instance().get_compute_stream()));
    }
    ...
    static size_t g_sort_temp_storage_bytes = 0;
    static size_t g_sort_max_items          = 0; 
    ...
    void vec_sort_pairs_i4_c(void* d_keys_in, void* d_keys_buf, 
        ...
        // 1. DoubleBuffer Setup (Ping-Pong )
        ...
        // 2. Check & Reallocate Workspace if needed
        if (num_items > g_sort_max_items) {
            ...
            // Allocate with padding (x1.5) to prevent frequent re-alloc
        ...
        if (d_keys.Current() != (int*)((IDeviceVector<int>*)d_keys_in)->device_ptr()) {
             CUDA_CHECK(cudaMemcpyAsync(((IDeviceVector<int>*)d_keys_in)->device_ptr(), 
                                        d_keys.Current(), num_items * sizeof(int), cudaMemcpyDeviceToDevice, stream));
             CUDA_CHECK(cudaMemcpyAsync(((IDeviceVector<int>*)d_vals_in)->device_ptr(), 
                                        d_values.Current(), num_items * sizeof(int), cudaMemcpyDeviceToDevice, stream));
        }
